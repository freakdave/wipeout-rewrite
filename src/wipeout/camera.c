...
void camera_update_race_intro(camera_t *camera, ship_t *ship, droid_t *droid) {
	...
	pos.x += sinf(( (ship->update_timer - UPDATE_TIME_RACE_VIEW) * 30 * 3.0 * twopi_i754) / 4096.0) * 4096;
	pos.y -= (2 *  (ship->update_timer - UPDATE_TIME_RACE_VIEW) * 30) + 200;
	pos.z += sinf(( (ship->update_timer - UPDATE_TIME_RACE_VIEW) * 30 * 3.0 * twopi_i754) / 4096.0) * 4096;
...
void camera_update_attract_circle(camera_t *camera, ship_t *ship, droid_t *droid) {
	...
	camera->position.y = ship->position.y + ((ship->angle.x * 512 / (twopi_i754)) - 200);
